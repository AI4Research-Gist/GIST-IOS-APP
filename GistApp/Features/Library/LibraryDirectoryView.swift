...
private struct ProjectDirectoryCard: View {
  @Environment(GistTheme.self) private var theme
  let project: Project
    ...
    Button(action: action) {
      VStack(alignment: .leading, spacing: theme.spacing.sm) {
        HStack(alignment: .firstTextBaseline, spacing: theme.spacing.md) {
          ...
          Spacer()
          Text(project.isActive ? "活跃中" : "已归档")
            .font(theme.fonts.caption2)
            .foregroundStyle(project.isActive ? theme.colors.accentSecondary : theme.colors.textTertiary)
        }
        ...
        Text("更新于 \(stats.lastUpdatedAt.formatted(.relative(presentation: .named)))")
          .font(theme.fonts.caption2)
          .foregroundStyle(theme.colors.textTertiary)
      }
      .gistCard()
