...
private struct HomeInsightCard: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: theme.spacing.md) {
      Text("灵感漫游")
        .font(theme.fonts.headline)
        .foregroundStyle(theme.colors.textPrimary)
      if let item = weightedRandomInsight {
  ...
  private var weightedRandomInsight: ResearchItem? {
    guard !items.isEmpty else { return nil }

    let now = Date()
    let weightedItems = items.map { item -> (item: ResearchItem, weight: Double) in
      let age = max(now.timeIntervalSince(item.updatedAt), 60)
      let recencyWeight = max(1, 7 * 86_400 / age)
      let starWeight = item.isStarred ? 1.6 : 1.0
      return (item, recencyWeight * starWeight)
    }
